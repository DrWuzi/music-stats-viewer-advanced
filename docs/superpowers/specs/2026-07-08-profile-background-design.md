...
All animated patterns are wrapped in `@media (prefers-reduced-motion: no-preference)` so motion-sensitive users never see movement, matching the existing avatar-decoration animations' accessibility posture.

## Rendering architecture

- New `lib/profile-backgrounds.ts`: `ProfileBackgroundKey` type, `PROFILE_BACKGROUND_KEYS`, `PROFILE_BACKGROUND_LABELS`, `isValidProfileBackground()` — mirrors `lib/profile-themes.ts`.
